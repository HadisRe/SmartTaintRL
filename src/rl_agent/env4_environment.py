# environment.py (اصلاح شده برای gymnasium)
"""
BadRandomness Detection RL Environment
محیط اصلی برای training agent
"""
...
class BadRandomnessEnv(gym.Env):
    ...
    #     """
    #     محاسبه reward با هدف pruning 30% و حفظ دقت 90%
    #     بر اساس importance range واقعی: 0.15-0.45
    #     """
    #
    #     # =================================================================
    #     # SECTION 1: تنظیمات اولیه و Thresholds واقعی
    #     # =================================================================
    #
    #     # Thresholds بر اساس داده واقعی (percentiles)
    #     # این اعداد از تحلیل 8000+ path واقعی استخراج شده
    #     VERY_HIGH_IMP = 0.38  # P85 - فقط 15% paths بالاتر از این
    #     HIGH_IMP = 0.32  # P70 - 30% paths بالاتر از این
    #     MED_IMP = 0.27  # P50 - میانه importance
    #     LOW_IMP = 0.22  # P30 - 70% paths بالاتر از این
    #     VERY_LOW_IMP = 0.18  # P15 - فقط 15% paths پایین‌تر از این
    #
    #     # شروع با reward صفر
    #     base_reward = 0.0
    #
    #     # =================================================================
    #     # SECTION 2: Contract-Aware Baseline (تأثیر قوی‌تر)
    #     # =================================================================
    #
    #     if hasattr(self.episode_info, 'contract_label'):
    #         contract_label = self.episode_info.contract_label
    #
    #         if contract_label == 'SAFE':
    #             # در contracts امن، SKIP را قویاً تشویق کن
    #             if action_type == 'SKIP':
    #                 base_reward += 4.0
    #             else:  # ANALYZE
    #                 base_reward -= 2.0
    #
    #         elif contract_label == 'VULNERABLE':
    #             # در contracts آسیب‌پذیر، دقیق‌تر باش
    #             if action_type == 'ANALYZE' and importance > MED_IMP:
    #                 base_reward += 2.0
    #             elif action_type == 'SKIP' and importance < LOW_IMP:
    #                 base_reward += 1.0
    #             else:
    #                 base_reward -= 1.0
    #
    #     # =================================================================
    #     # SECTION 3: Importance-Based Rewards (با range واقعی)
    #     # =================================================================
    #
    #     if action_type == 'ANALYZE':
    #         # فقط paths واقعاً مهم را تشویق کن
    #         if importance > VERY_HIGH_IMP:
    #             # Top 15% - حتماً باید آنالیز شود
    #             base_reward += 8.0
    #         elif importance > HIGH_IMP:
    #             # Top 30% - احتمالاً مهم
    #             base_reward += 3.0
    #         elif importance > MED_IMP:
    #             # بالای میانه - neutral
    #             base_reward += 0.0
    #         elif importance > LOW_IMP:
    #             # زیر میانه - کمی منفی
    #             base_reward -= 2.0
    #         else:
    #             # Bottom 30% - نباید آنالیز شود
    #             base_reward -= 5.0
    #
    #     else:  # SKIP
    #         # paths کم‌اهمیت را برای skip تشویق کن
    #         if importance < VERY_LOW_IMP:
    #             # Bottom 15% - حتماً skip شود
    #             base_reward += 8.0
    #         elif importance < LOW_IMP:
    #             # Bottom 30% - احتمالاً بی‌اهمیت
    #             base_reward += 4.0
    #         elif importance < MED_IMP:
    #             # زیر میانه - مثبت خفیف
    #             base_reward += 1.0
    ...
    #             base_reward += 0.0
    #         else:
    #             # Top 30% - نباید skip شود
    #             base_reward -= 6.0
    #
    #     # =================================================================
    #     # SECTION 4: Risk-Based Adjustments (واضح و قاطع)
    #     # =================================================================
    #
    #     if path_risk == 'HIGH':
    #         if action_type == 'ANALYZE':
    #             base_reward += 5.0  # True Positive - reward قوی
    #         else:  # SKIP
    #             base_reward -= 8.0  # False Negative - penalty شدید
    #
    #     elif path_risk == 'LOW':
    #         if action_type == 'SKIP':
    #             base_reward += 5.0  # True Negative - reward قوی
    #         else:  # ANALYZE
    #             base_reward -= 4.0  # False Positive - penalty متوسط
    #
    #     elif path_risk == 'MEDIUM':
    #         # برای MEDIUM، به importance اعتماد کن
    #         if importance > MED_IMP:
    #             # MEDIUM با importance بالا
    #             if action_type == 'ANALYZE':
    #                 base_reward += 2.0
    #             else:
    #                 base_reward -= 3.0
    #         else:
    #             # MEDIUM با importance پایین
    #             if action_type == 'SKIP':
    #                 base_reward += 2.0
    #             else:
    #                 base_reward -= 2.0
    #
    #     # =================================================================
    #     # SECTION 5: Pattern Discovery Bonus
    #     # =================================================================
    #
    #     pattern_info = path.get('pattern_info', {})
    #     if action_type == 'ANALYZE' and pattern_info.get('is_new', False):
    #         # patterns جدید ارزش کشف دارند
    #         base_reward += 3.0
    #         # در اوایل training، bonus بیشتر
    #         if len(self.discovered_patterns) < 30:
    #             base_reward += 2.0
    #
    #     # =================================================================
    #     # SECTION 6: Pruning Rate Management (حیاتی برای هدف 30%)
    #     # =================================================================
    #
    #     if self.episode_info.steps >= 10:
    #         analyze_ratio = self.episode_info.paths_analyzed / self.episode_info.steps
    #         skip_ratio = 1 - analyze_ratio
    #
    #         # هدف: 70% analyze, 30% skip
    #         target_analyze = 0.7
    #         target_skip = 0.3
    #
    #         # Reward/Penalty بر اساس فاصله از هدف
    #         if action_type == 'ANALYZE':
    #             if analyze_ratio > 0.85:
    #                 # بیش از حد analyze می‌کند
    #                 base_reward -= (analyze_ratio - 0.85) * 20
    #             elif analyze_ratio < 0.55:
    #                 # کمتر از حد analyze می‌کند
    #                 base_reward += (0.55 - analyze_ratio) * 10
    #
    #         else:  # SKIP
    #             if skip_ratio < 0.15:
    #                 # خیلی کم skip می‌کند - bonus قوی
    #                 base_reward += 5.0
    #             elif skip_ratio < 0.25:
    #                 # کم skip می‌کند - bonus
    #                 base_reward += 3.0
    #             elif skip_ratio > 0.45:
    #                 # بیش از حد skip می‌کند - penalty
    #                 base_reward -= (skip_ratio - 0.45) * 15
    #
    #     # =================================================================
    #     # SECTION 7: Early Episode Bonus (کمک به exploration)
    #     # =================================================================
    #
    #     if self.episode_info.steps < 5:
    #         # در قدم‌های اول، تصمیمات درست را تشویق کن
    #         if (action_type == 'ANALYZE' and importance > HIGH_IMP) or \
    #                 (action_type == 'SKIP' and importance < LOW_IMP):
    #             base_reward += 2.0
    #
    #     # =================================================================
    #     # SECTION 8: Consistency Check (جلوگیری از رفتار random)
    #     # =================================================================
    #
    #     # اگر importance خیلی واضح است اما تصمیم اشتباه گرفته
    #     if importance > VERY_HIGH_IMP and action_type == 'SKIP':
    #         base_reward -= 10.0  # penalty شدید
    #     elif importance < VERY_LOW_IMP and action_type == 'ANALYZE':
    #         base_reward -= 8.0  # penalty شدید
    #
    #     # =================================================================
    #     # Clip final reward برای stability
    #     # =================================================================
    #
    #     return np.clip(base_reward, -15.0, 15.0)
